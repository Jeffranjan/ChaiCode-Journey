...
function generateReportCard(student) {
  ...
  if (
    ...
    const totalMarks = Object.values(student.marks).reduce((total, item) => {
      return total + item;
    }, 0);
    ...
    const highestSubjectName = highestSubject[0];
    ...
    const lowestSubjectName = lowestSubject[0];

    const failedSubjects = Object.entries(student.marks)
      .filter(([subject, score]) => score < 40)
...
const student = {
  name: "Rahul",
  marks: { maths: 85, science: 92, english: 30 },
};
